...
export default function Popup() {
  ...
  React.useEffect(() => {
    chrome.storage.local.get(['openaiApiKey', 'totalRequests', 'lastUsed'], (res) => {
  ...
  return (
    <div style={{ width: 320, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: 'oklch(0.97 0.008 60)' }}>
      {/* Header */}
      ...
      {isConnected && !editing ? (
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          ...
          {/* Model */}
          <div style={{ background: 'white', borderRadius: 12, padding: '11px 14px', border: '1px solid oklch(0.91 0.006 60)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'oklch(0.45 0.03 40)' }}>Model</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: 'oklch(0.18 0.02 40)' }}>GPT-4o Vision</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setEditing(true)} style={{ flex: 1, height: 36, borderRadius: 10, border: '1px solid oklch(0.88 0.01 60)', background: 'white', fontSize: 12, fontWeight: 600, color: 'oklch(0.35 0.02 40)', cursor: 'pointer', fontFamily: 'inherit' }}>
              Edit key
            </button>
            <button onClick={clear} style={{ flex: 1, height: 36, borderRadius: 10, border: 'none', background: 'oklch(0.95 0.015 25)', fontSize: 12, fontWeight: 600, color: 'oklch(0.5 0.12 25)', cursor: 'pointer', fontFamily: 'inherit' }}>
              Disconnect
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          ...
          <input
            ...
            onKeyDown={handleKey}
            placeholder="sk-..."
            style={{ width: '100%', height: 38, padding: '0 12px', borderRadius: 10, border: '1px solid oklch(0.88 0.01 60)', background: 'white', fontSize: 13, fontFamily: 'inherit', outline: 'none', color: 'oklch(0.18 0.02 40)', marginBottom: 10, boxSizing: 'border-box' }}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {editing && (
              <button onClick={() => { setEditing(false); setKey(''); }} style={{ height: 36, padding: '0 16px', borderRadius: 10, border: '1px solid oklch(0.88 0.01 60)', background: 'white', fontSize: 12, fontWeight: 600, color: 'oklch(0.5 0.03 40)', cursor: 'pointer', fontFamily: 'inherit' }}>
                Cancel
              </button>
            )}
            <button onClick={save} disabled={!key.trim()}
              style={{ flex: 1, height: 36, borderRadius: 10, border: 'none', background: key.trim() ? `linear-gradient(135deg, ${O}, oklch(0.72 0.15 55))` : 'oklch(0.9 0.005 60)', color: key.trim() ? 'white' : 'oklch(0.6 0.02 40)', fontSize: 13, fontWeight: 600, cursor: key.trim() ? 'pointer' : 'default', fontFamily: 'inherit', boxShadow: key.trim() ? `0 4px 14px ${OG}` : 'none' }}>
